...
- Return ONLY a valid JSON object with no markdown or code fences:
  {{"response": "def flatten_columns(df: pd.DataFrame) -> pd.DataFrame:\\n    ...",
    ...
    df: pd.DataFrame = llm_exec_with_retry(
...
#     """
#     report = {"numerical_cols_processed": [], "conversions_applied": {}}
#     # Pre-detect which columns need treatment
#     candidate_cols = {}
...
# Candidate numerical columns and their detected patterns:
# {candidate_cols}
...
# Uses ONLY pandas, re, and numpy standard libraries.
...
#     df, conversions_applied = llm_exec_with_retry(
...
#         max_retries=3,
#     )
#     report["numerical_cols_processed"] = list(candidate_cols.keys())
#     report["conversions_applied"] = conversions_applied
#     return df, report
...
{{
    ...
    "reasoning": "2 sentence max explaining the approach."
        ...
        try:
            df, llm_conversions = llm_exec_with_retry(
                fn_name="standardize_numerical",
                messages=messages,
...
def standard_data_pipeline(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = _handle_hierarchical_columns(df)
